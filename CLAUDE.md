...
- 初回公開など CI トークンで 404 になる場合は、各パッケージルート（`packages/sync`・`packages/blocks`）で `pnpm run release:local`（`npm_config_provenance=false pnpm publish --no-git-checks`）を手動実行するか、ルートで `pnpm release:local`（`npm_config_provenance=false pnpm -r publish --no-git-checks`）を実行する。**`pnpm publish` に `--no-provenance` フラグは存在しない**（npm CLI 固有のフラグで pnpm には無い）ため、`publishConfig.provenance: true` を無効化するには `npm_config_provenance=false` 環境変数を使う。
