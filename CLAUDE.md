...
<!-- confirmed 2026-07-10: EmDash 公式ドキュメント（reference/mcp-server）は「セッションクッキーも動くが外部 MCP クライアントには非実用的」と書いているが、これは不正確（emdash 本体 packages/core/src/astro/middleware/auth.ts:271-278 で実装・実測確認）。MCP エンドポイント（`/_emdash/api/mcp`）は bearer-only 設計で、`isMcpEndpoint && !isTokenAuth` なら session/cookie 認証を一切参照せず 401 `NOT_AUTHENTICATED`（WWW-Authenticate 付き OAuth discovery 401）を返す。`/_emdash/api/setup/dev-bypass?redirect=/_emdash/admin`（`emdash login` が localhost かつ認証未設定時に自動使用するローカル専用バイパス）で取得した `astro-session` クッキーは `/_emdash/api/auth/me`・`/_emdash/admin` には通る（200、dev@emdash.local・role 50）が、MCP には通らない。MCP を叩くには PAT（`ec_pat_*`）か OAuth/Device Flow の Bearer トークンが必須。 -->
...
3. **プラグインの管理画面ルート（Block Kit の admin action）を `curl` で直接叩くと `CSRF_REJECTED` で 403 になる**（2026-07-09 確認）。セッションクッキーだけでは不十分で、ブラウザ経由のダブルサブミット等の CSRF トークンが要る。curl だけで完結させたい検証には、代わりに `emdash` CLI（`emdash content create/get/delete/restore` 等、Bearer/CLI 認証）や MCP サーバーを使う方が早い。**ただしこれはセッションクッキー限定の話**（2026-07-10 追加確認、`packages/core/src/astro/routes/api/plugins/[pluginId]/[...path].ts:65`）。Bearer トークン（PAT 等、`locals.tokenScopes` が立つ）で叩く場合はこの CSRF チェック自体が丸ごとスキップされるため、`X-EmDash-Request` ヘッダーなしで `curl -H "Authorization: Bearer ec_pat_..." http://localhost:4321/_emdash/api/plugins/notion-sync/admin` が直接 200 で通る（`plugins:manage` 権限 + token scope `admin` は必要）。MCP の 45 固定ツールにはプラグイン設定を読むツールが無い（[[local-mcp-access]] 参照）ので、notion-sync の設定内容（Notion トークン等の secret は値がマスクされ返らない、マッピング一覧・fetch 済み構造など非 secret な状態）を読みたい場合はこの `.../admin` ルートを Bearer PAT で叩くのが実質唯一の経路。
...
5. **`release-stable.yml`（`changesets/action` 経由の正式 publish）が `NODE_AUTH_TOKEN` を渡さず `NPM_TOKEN` のみだと npm publish が（401/403 ではなく）`E404` で失敗する**（2026-07-10 確認）。`actions/setup-node` が生成する `.npmrc` の `_authToken` 変数展開は `NODE_AUTH_TOKEN` という環境変数名を見るため、`NPM_TOKEN` だけでは未認証扱いになり、scoped package への操作が権限エラーではなく "存在しない" 体で 404 を返す（npm の意図的な仕様）。**さらに悪いことに `release.yml`（canary）側にはこの実害を隠す設計欠陥があった**: pending changeset が 0 件（= Version Packages PR マージ直後）の push では `pnpm version:snapshot` が package.json の「正式な」バージョン番号（例 `0.3.1`）をバンプせずそのまま素通りさせ、後続の `pnpm release:snapshot`（`changeset publish --tag canary`）がその正式バージョン番号を **`canary` タグの下で** publish してしまう。stable 側が E404 で失敗する一方 canary 側はこれで「成功」するため、正式バージョン番号が registry に「公開済み」として記録され、以後 `changeset publish`（正式 `latest` 公開）は毎回「既に公開済み」と判定して恒久的に no-op になる（`latest` タグが古いバージョンのまま取り残される）。旧コメント「pending changeset が 0 件なら … release:snapshot も publish 対象なしで no-op 終了する（安全）」は誤りだった。対策: `release-stable.yml` に `NODE_AUTH_TOKEN` を追加し、`release.yml` は pending changeset の有無（`.changeset/*.md`、`README.md` 除く）を事前チェックして 0 件なら canary ジョブ全体をスキップする（正式公開は `release-stable.yml` だけに一本化する）。**この不具合で `latest` タグが古いまま取り残された場合、`changeset publish` の再実行では直らない**（該当バージョンが既に registry に存在するため）。`npm dist-tag add <pkg>@<version> latest` で手動修復するか、新たな patch バージョンで publish し直す必要がある。
...
- **Version PR（自動）**: `main` push ごとに `release-stable.yml` の `changesets/action` が走り、pending changeset があれば「Version Packages」PR を自動作成/更新する（publish はしない）。feature マージ時は canary 公開とこの PR 作成が同じ push で同時に起きる。手動で回したい場合は `Release Stable` の workflow_dispatch も使える。
- **stable（自動 publish）**: 上記「Version Packages」PR をマージすると、pending changeset が 0 件になった状態で `release-stable.yml` が再度走り、対象パッケージを `latest` へ公開する（このとき canary は no-op）。
