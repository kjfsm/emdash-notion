...
2. **changeset の bump 種別**: 明示的な指示がない限り **`patch`** を使う（`minor` は後方互換の公開 API 追加、`major` は破壊的変更のみ）。`packages/**` を変更する PR には changeset を必須（`skip-changeset` ラベルで免除）。
3. **descriptor の `version` は package.json を単一情報源とする**。`src/version.ts`（`scripts/gen-version.mjs` が生成・各パッケージの `prebuild` で自動再生成、手編集禁止）から `VERSION` を import して descriptor に渡す。ハードコード禁止（マニフェスト上のバージョンが乖離する）。
...
5. **プラグイン `id`** は `packages/sync` が `notion-sync`（webhook URL `/_emdash/api/plugins/notion-sync/webhook`・storage・admin.pages）、`packages/blocks` が `notion-blocks`。
