...
export async function syncAll(ctx: PluginContext): Promise<BulkSyncResult> {
	const config = await loadConfig(ctx);
	const result: BulkSyncResult = { total: 0, created: 0, updated: 0, unchanged: 0, skipped: 0, failed: 0, errors: [] };

	if (!isConfigReady(config)) {
		result.errors.push("プラグイン未設定（Notion トークン / コレクションとデータベースの対応を入力してください）");
		return result;
	}
	if (!ctx.http) {
		...
		do {
			...
			for (const notionPage of page.results) {
				...
				try {
					...
					result.failed++;
					result.errors.push(`${notionPage.id}: ${err instanceof Error ? err.message : String(err)}`);
					ctx.log.warn("manual sync: page ingest failed", { pageId: notionPage.id, error: String(err) });
				}
			}
