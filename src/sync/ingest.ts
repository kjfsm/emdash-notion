...
export async function ingestPage(ctx: PluginContext, pageId: string): Promise<IngestResult> {
	...
	ctx.log.info("notion page synced", { pageId: page.id, emdashId, status, published, collection: mapping.collection });
	...
	for (;;) {
		try {
			...
			if (missingField && missingField in remaining) {
				ctx.log.warn("notion sync: dropping field not present in collection schema", { pageId, field: missingField });
