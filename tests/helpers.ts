...
export function mapStorage() {
	...
	return {
		...
		collection: {
			...
			getMany: async (ids: string[]) => new Map(ids.filter((i) => store.has(i)).map((i) => [i, store.get(i)])),
...
export interface TestContextOptions {
	...
	onUpload?: (filename: string, contentType: string, bytes: ArrayBuffer) => { mediaId: string; url: string };
}
	...
	const ctx = {
		...
		content: {
			...
			create: async (collection: string, data: Record<string, unknown>) => {
				...
				return { id: item.id, type: collection, slug: null, status: "draft", data, locale: "en", createdAt: "", updatedAt: "", publishedAt: null };
			},
			update: async (collection: string, id: string, data: Record<string, unknown>) => {
				...
				return { id, type: collection, slug: null, status: "draft", data, locale: "en", createdAt: "", updatedAt: "", publishedAt: null };
			},
			delete: async () => true,
		},
		media: {
			...
			upload: async (filename: string, contentType: string, bytes: ArrayBuffer) => {
				const r = options.onUpload?.(filename, contentType, bytes) ?? { mediaId: `media_${++idSeq}`, url: `https://cdn.example.com/${filename}` };
